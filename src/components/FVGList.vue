<template>
  <div class="glass rounded-xl p-4 flex flex-col gap-2 h-full">
    <div class="flex items-center justify-between mb-1">
      <p class="text-xs font-semibold text-[#e2e8f0] tracking-wide">🐋 Fair Value Gaps (SMC)</p>
      <span v-if="loading" class="w-3 h-3 rounded-full border-2 border-[#3b82f6] border-t-transparent animate-spin" />
    </div>

    <div v-if="!fvgs.length && !loading" class="flex-1 flex items-center justify-center">
